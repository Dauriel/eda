...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    public final double factorCarga () {
        double counter = 0.0;
        ListaConPI<EntradaHash<C, V>> list;
        for(int i = 0; i < elArray.length; i++){
            list = elArray[i];
            list.inicio();
            while(!list.esFin()){
    ...
    protected final void rehashing() { //REPASAR
        ...
        ListaConPI<EntradaHash<C, V>> list;
        EntradaHash<C,V> entry;
        for(int i = 0; i < aux.length; i++){
            list = aux[i];
            for(list.inicio(); !list.esFin(); list.siguiente()){
                entry = list.recuperar();
                elArray[indiceHash(entry.clave)].insertar(new EntradaHash<C,V>(entry.clave, entry.valor));
            }
        }
    } 
    ...
    public ListaConPI<C> claves() {
        ListaConPI<C> list = new LEGListaConPI<C>();
        for(int i = 0; i < elArray.length; i++){
            ListaConPI<EntradaHash<C,V>> aux = elArray[i];
            for(aux.inicio(); !aux.esFin(); aux.siguiente()){
                list.insertar(aux.recuperar().clave);
            }
        }
        return list;
    }
    ...
    public final double desviacionTipica() {
        int numEntries = 0;
        for(int i = 0; i < elArray.length; i++){
            numEntries += elArray[i].talla();
        }
        double averageLength = ((1.0)* numEntries) / elArray.length;
        
        double sum = 0.0;
        for(int i = 0; i < elArray.length; i++){
            sum += (elArray[i].talla() - averageLength) * (elArray[i].talla() - averageLength);
        }
    ...
    public String histograma() {
        String res = "";
        int[] bucketLength = arrayBucketLength();
        for(int i = 0; i <= 9; i++){
            res += i + "\t" + bucketLength[i] + "\n";
        }
        return res;
    }
    
    public int[] arrayBucketLength(){
        int[] res = new int[10];
        for(int i = 0; i < elArray.length; i++){
